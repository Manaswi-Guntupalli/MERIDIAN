      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="flex flex-col items-center justify-center py-10 text-center">
          <div className="mb-4 inline-flex rounded-xl border border-white/10 p-1">
            {(['RFID', 'CV'] as const).map((m) => (
              <button key={m} onClick={() => setMode(m)} className={`flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-semibold transition ${mode === m ? 'bg-brand-gradient text-ink-950' : 'text-slate-400'}`}>
          ...
          <div className="relative my-6 grid h-44 w-44 place-items-center">
            ...
            <div className="grid h-28 w-28 place-items-center rounded-full bg-brand-gradient text-ink-950">
              {mode === 'RFID' ? <Radio className="h-10 w-10" /> : <ScanFace className="h-10 w-10" />}
            </div>
          </div>

          <div className="text-sm font-semibold text-white">{scanning ? `${mode === 'CV' ? 'Matching embedding…' : 'Reading tag…'}` : `Ready — tap a student below`}</div>
          ...
          <div className="mt-6 flex max-h-40 flex-wrap justify-center gap-1.5 overflow-y-auto no-scrollbar">
            {students.data?.slice(0, 16).map((s) => (
              <button key={s.id} onClick={() => simulateTap(s)} disabled={scanning} className="chip glass-hover hover:!text-white">
        ...
        <Card className="!p-0">
          <div className="flex items-center gap-2 border-b border-white/[0.06] px-5 py-4">
            ...
            <h2 className="font-bold text-white">Live attendance stream</h2>
          </div>
          <div className="max-h-[26rem] overflow-y-auto no-scrollbar p-3">
            <AnimatePresence initial={false}>
              {taps.length === 0 ? (
                ...
                <motion.div key={t.at + i} layout initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} className="mb-2 flex items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
                  <div className="grid h-9 w-9 place-items-center rounded-lg bg-brand-gradient text-xs font-bold text-ink-950">{initials(t.name)}</div>
