  ...
  return (
    <div>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        <Card className="flex items-start gap-3"><span className="grid h-9 w-9 place-items-center rounded-lg bg-white/5 text-brand-400"><Server className="h-4 w-4" /></span><div><div className="text-sm font-semibold text-white">{user.school?.name ?? 'Meridian'}</div><div className="text-xs text-slate-500">Code: {user.school?.code}</div></div></Card>
        <Card className="flex items-start gap-3"><span className="grid h-9 w-9 place-items-center rounded-lg bg-white/5 text-cyan-400"><Database className="h-4 w-4" /></span><div><div className="text-sm font-semibold text-white">Event-sourced core</div><div className="text-xs text-slate-500">Append-only · Time Machine</div></div></Card>
        <Card className="flex items-start gap-3"><span className="grid h-9 w-9 place-items-center rounded-lg bg-white/5 text-mint-400"><Cpu className="h-4 w-4" /></span><div><div className="text-sm font-semibold text-white">AI engines</div><div className="text-xs text-slate-500">Lumen · Kairos · Foresight · Copilot · Presence</div></div></Card>
      </div>

      <Card className="!p-0">
        <div className="flex items-center gap-2 border-b border-white/[0.06] px-5 py-4">
          <ShieldCheck className="h-4 w-4 text-brand-400" />
          <h2 className="font-bold text-white">Role-based access control</h2>
          ...
          <table className="w-full min-w-[640px] text-sm">
            <thead>
              <tr className="border-b border-white/[0.06]">
                ...
                {ROLES.map((r) => (
                  <th key={r} className="px-2 py-3 text-center text-[0.65rem] font-semibold uppercase tracking-wider text-slate-400">{r}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {CAPS.map((row, i) => (
                <motion.tr key={row.area} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }} className="border-b border-white/[0.03]">
                  <td className="px-5 py-2.5 text-slate-300">{row.area}</td>
                  {row.allow.map((ok, j) => (
                    <td key={j} className="px-2 py-2.5 text-center">
                      {ok ? (
                        ...
                        <span className="inline-grid h-6 w-6 place-items-center rounded-md bg-white/[0.03] text-slate-700"><Minus className="h-3.5 w-3.5" /></span>
                      )}
                    </td>
                  ))}
                </motion.tr>
