...
function TimeMachine() {
  ...
  const snap = useQuery({
    queryKey: ['activity-replay', atISO],
    queryFn: async () => (await api.get('/trust/activity-replay', { params: { at: atISO } })).data,
  ...
  return (
    <Card>
      <div className="mb-2 flex items-center justify-between">
        <div className="flex items-center gap-2"><History className="h-4 w-4 text-brand-400" /><span className="font-bold text-slate-900">Activity replay</span></div>
        <Badge severity="INFO">{new Date(atISO).toLocaleString()}</Badge>
      </div>
      <p className="mb-5 text-sm text-slate-500">
        Drag the slider to fold the append-only event log up to any past moment and see what had happened by then
        (reverted events excluded). Scope today: recorded activity — not yet a full replay of every table.
      </p>
...
function Snap({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-line bg-ink-800/60 p-3 text-center">
      <motion.div key={value} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="text-2xl font-extrabold text-slate-900">{value}</motion.div>
  ...
  return (
    <Card className="!p-0">
      <div className="relative px-5 py-4">
        <div className="absolute bottom-0 left-[38px] top-14 w-px bg-line" />
          ...
          {events.data?.map((e, i) => (
            <motion.div key={e.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: Math.min(i * 0.02, 0.3) }} className="relative flex items-center gap-4 py-2.5">
              ...
              <div className="min-w-0 flex-1">
                <div className={cn('text-sm font-semibold', e.reverted ? 'text-slate-500 line-through' : 'text-slate-900')}>{prettyType(e.type)}</div>
                <div className="text-xs text-slate-500">{e.actorName ?? 'system'} · {timeAgo(e.createdAt)} · <span className="font-mono text-slate-400">{e.aggregate}</span></div>
              </div>
      ...
      {logs.data?.length ? logs.data.map((l, i) => (
        <motion.div key={l.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.02, 0.3) }}>
          <Card className="flex items-center gap-4 !py-3.5">
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-ink-800"><Sparkles className={cn('h-4 w-4', engineColor[l.engine])} /></span>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2"><span className={cn('text-[0.65rem] font-bold uppercase tracking-wider', engineColor[l.engine])}>{l.engine}</span><span className="text-sm font-semibold text-slate-900">{l.action}</span></div>
            ...
            <span className="hidden text-[0.65rem] text-slate-400 sm:inline">{timeAgo(l.createdAt)}</span>
          </Card>
        </motion.div>
